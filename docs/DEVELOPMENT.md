...
当前正式版本为 v1.0.0。插件的数据源为 `zbp_xz_visit_stats_log`（实际表前缀由 Z-BlogPHP 数据库配置决定）。
...
1. 对访问记录验证筛选、分页、详情和 HTML 转义。
2. 对概览、蜘蛛、来源和 IP 页面，将核心指标与同范围直接 SQL 聚合结果比较。
3. 选择无日志的自定义时间段，确认指标为 0、列表为空、不出现 `NaN` 或 `Infinity`。
4. 在 1100px 与 680px 宽度下检查筛选、指标卡、图表和表格横向滚动。
5. 对 IP 分析验证普通访问、高频访问、404 请求和扫描工具 UA 命中规则；异常结果只用于展示。
...
`plugin.xml` 的版本字段已同步为 `1.0.0`。发布前仍应核对版本、说明、发布日期、兼容性声明和后台登录态人工验收；本轮发布整理不修改运行时数据结构。
