...
function xz_visit_stats_upgrade_migrate_to_20()
{
    ...
    xz_visit_stats_upgrade_create_page_uv_table();
...
function xz_visit_stats_upgrade_create_pages_table()
{
    ...
    if ($zbp->db->ExistTable($table)) return;
...
function xz_visit_stats_upgrade_create_page_uv_table()
{
    global $zbp;
    $table = $zbp->db->dbpre . 'xz_visit_stats_page_uv';
    if ($zbp->db->ExistTable($table)) return;
    $sql = $zbp->db->sql->CreateTable($table, array(
        'ID'=>array('ID','integer','bigint',0),
        'Url'=>array('Url','string','text',''),
        'VisitorHash'=>array('VisitorHash','string',128,''),
        'Created'=>array('Created','integer','bigint',0),
    ));
    $zbp->db->QueryMulti($sql);
}
